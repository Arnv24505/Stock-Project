...
import usersData from "../../Data/mockUser.json"; // Import the users data
import {
  Chart as ChartJS,
  LineElement,
  CategoryScale,
...
import { BsBell, BsSearch, BsList, BsChevronLeft } from "react-icons/bs";
...
// We'll pass a userId as a prop to this component
const UserDashboard = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    // This would also be where you fetch user-specific stocks or other data
    setStocks(stocksData.stocks || []);
  }, [userId]); // The useEffect hook will re-run whenever the userId prop changes

  // Display a loading or not found message if the user isn't loaded yet
  if (!user) {
    return <div className="flex h-screen items-center justify-center text-xl text-gray-700">User not found or loading...</div>;
  }
        ...
        <div className="h-16 bg-white shadow-sm flex items-center justify-between px-8">
          ...
          <div className="flex items-center">
            ...
            <h2 className="text-lg font-medium text-gray-800">
              Hello, <span className="text-emerald-600">{user.firstName} {user.lastName}</span>
            </h2>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="relative group">
              ...
              <div className="absolute top-12 -right-4 w-64 bg-white rounded-lg shadow-lg p-4 z-50 hidden group-hover:block transition-all duration-300">
                <h4 className="font-semibold text-gray-800 mb-2">Notifications</h4>
                <ul>
                  <li className="text-sm text-gray-600 border-b pb-2 mb-2 last:border-b-0">
                    StockTracker: Your portfolio performance is up 1.2%!
                  </li>
                  <li className="text-sm text-gray-600">
                    Alert: Apple stock has dropped below your target price.
                  </li>
                </ul>
              </div>
            </div>
            ...
            <button className="flex items-center">
              <img
                src={`https://ui-avatars.com/api/?name=${encodeURIComponent(user.firstName)}+${encodeURIComponent(user.lastName)}&background=10b981&color=fff`}
                alt="Profile"
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-500">{stock.symbol || 'N/A'}</div>
                          </div>
                        </div>
                      </td>
                      <td className="px-6 py-4 text-gray-900">{stock.quantity || '-'}</td>
                      <td className="px-6 py-4 text-gray-900">${(stock.avgPrice || 0).toFixed(2)}</td>
                      <td className="px-6 py-4 text-gray-900">${(stock.currentPrice || 0).toFixed(2)}</td>
                      <td className="px-6 py-4">
                        <span
                          className={`${
